...
void Ai2player::receive_global_settings(Global_Settings* gs) {
    //world_map = gs.graph; Check header files before calling private fields!
    this->world_map = gs->get_graph();
}

Dispatch_Move Ai2player::first_move(Initial_Settings *is) {

    Dispatch_Move moveToSend;
    map<Country*, int> distribution;

    // int nt = is.territories[this].size();

    int nb_territories = (is->territories(this)->second).size();
    vector<Country*> my_territories = (is->territories(this))->second;
    ...
    int random_number;
    for (int i = 0; i < is.step.number_of_troops; i++) {
        random_number = rand() % nb_territories;
        distribution[my_territories[random_number]] += 1;
    }
    moveToSend.distribution = distribution;
    return moveToSend;
}

Move Random_Player::next_move(Game_State gs) {
    set<Card*> my_hand;
    my_hand = gs.hands[this];
    set<Card*> t[3];
