...
void Random_Player::receive_global_settings(Global_Settings gs) {
    world_map = gs.graph;
}

Move Random_Player::first_move(Initial_Settings is) {
    Dispatch_Move moveToSend;
    map<Country*, int> distribution;
    int nt = is.territories[this].size();
    Country* my_territories[nt];
    int i = 0;
    for (const auto& elem : is.territories[this]) {
        my_territories[i] = elem;
        i++;
    }
    for (int i = 0; i < nt; i++) {
